...
const GameLobby = () => {
  ...
  const joinGameIfNeeded = async (team, isSpymaster = false) => {
    try {
      ...
      if (!playerExists) {
        const joinRes = await fetch(`https://194.90.158.74/cgroup81/test2/tar1/api/playeringames/${gameId}/join`, {
          method: "POST",
  ...
  const updatePlayer = async (team, isSpymaster) => {
    try {
      await fetch(`https://194.90.158.74/cgroup81/test2/tar1/api/playeringames/${gameId}/update-player`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          gameID: parseInt(gameId),
  ...
  const checkIfReady = async () => {
    try {
      const res = await fetch(`https://194.90.158.74/cgroup81/test2/tar1/api/playeringames/${gameId}/is-ready`);
  ...
  const startGame = async () => {
    ...
    try {
      const res = await fetch(`https://194.90.158.74/cgroup81/test2/tar1/api/games/${gameId}/start`, { method: "POST" });
      const data = await res.json();
      if (!res.ok) {
